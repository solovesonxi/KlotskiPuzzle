...
public class LoginPanel extends JPanel {
    ...
    public LoginPanel(MainFrame mainFrame, int width, int height) {
        ...
        ViewUtil.createJLabel(contentPanel, new Point(width / 2 - 110, height / 2 - 100), 70, 30, 16, AppResources.get("label.username").concat(":")); // 用户名标签
        ViewUtil.createJLabel(contentPanel, new Point(width / 2 - 110, height / 2 - 50), 70, 30, 16, AppResources.get("label.password").concat(":")); // 密码标签
        ...
        JButton loginBtn = ViewUtil.createStyledButton(contentPanel, AppResources.get("button.login"), new Point(width / 2 - 80, height / 2 + 30), 80, 30, btnColor, btnFont);
        JButton registerBtn = ViewUtil.createStyledButton(contentPanel, AppResources.get("button.register"), new Point(width / 2 + 20, height / 2 + 30), 80, 30, btnColor, btnFont);
        JButton guestLoginBtn = ViewUtil.createStyledButton(contentPanel, AppResources.get("button.guest"), new Point(width / 2 - 80, height / 2 + 80), 180, 32, btnColor, btnFont);
    ...
    public boolean isValidCredentials(String user, String pass) {
        if (user.isEmpty() || pass.isEmpty()) { // 检查是否为空
            JOptionPane.showMessageDialog(this, AppResources.get("dialog.emptyfield"));
        } else if (Pattern.matches("^[a-zA-Z0-9一-龥]+$", user) && Pattern.matches("^[a-zA-Z0-9]+$", pass)) {
            return true; // 验证通过
        } else {
            JOptionPane.showMessageDialog(this, AppResources.get("dialog.syntax"));
        }
        return false;
    }
        ...
        try {
            if (isValidCredentials(user, new String(pass))) {
                ...
                mainFrame.showControl(user); // 显示控制面板
                    ...
                    JOptionPane.showMessageDialog(this, AppResources.get("dialog.invalid_login"));
                }
            }
        } catch (IOException exception) {
            JOptionPane.showMessageDialog(this, AppResources.get("dialog.fail_login") + exception.getMessage());
        } finally {
            Arrays.fill(pass, '\0');
        }
    }
        ...
        try {
            if (isValidCredentials(user, new String(pass))) {
                if (users != null && users.register(user, pass)) {
                    JOptionPane.showMessageDialog(this, AppResources.get("dialog.register_success"));
                    mainFrame.showControl(user);
                } else {
                    JOptionPane.showMessageDialog(this, AppResources.get("dialog.register_taken"));
                }
            }
        } catch (IOException exception) {
            JOptionPane.showMessageDialog(this, AppResources.get("dialog.register_fail") + exception.getMessage());
        } finally {
            Arrays.fill(pass, '\0');
        }
    }
