...
- A* 后台求解、搜索进度、取消与动画回放；
- 初步 Lab Mode：在单格步或整段移动规则下运行确定性的 BFS、贪心最佳优先、A* 和加权 A* 实验；
- 撤销、重新开始和 180 秒限时挑战；
- 本地玩家、游戏存档、步数榜和时间榜；
- 原创背景音乐，以及选中、移动、非法移动、撤销、胜利和失败音效。

当前 Lab Mode 已提供可复现实验配置、棋盘预览、实时计数、最终指标和取消能力。搜索图表、状态检查器、解法回放、实验记录与 HTML 报告仍属于 [v2 规划](docs/V2_PLAN.md) 中的后续工作。
